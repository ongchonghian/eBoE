  ...
  return (
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/5">
      <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.12em] text-slate-500">TrustVC x eBoE</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Stat title="Existing" value={counts.existing} toneClass="text-emerald-700 border-emerald-200 bg-emerald-50" />
...
function Stat({ title, value, toneClass = "text-slate-900 border-slate-200 bg-white" }) {
  return (
    <div className={`rounded-2xl border p-4 ${toneClass}`}>
      ...
      <div className="mt-1 text-sm font-semibold uppercase tracking-[0.12em] opacity-70">{title}</div>
    </div>
  );
}
...
export function StoryRail({ chapter, setChapter, activeIndex, setActiveIndex }) {
  ...
  return (
    <aside className="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-xl shadow-slate-900/5">
      <div className="mb-4 text-sm font-semibold uppercase tracking-[0.12em] text-slate-500">Chapters</div>
      <div className="space-y-2">
        {chapters.map(([id, act, label, icon]) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.1em] opacity-80">
      ...
      <div className="mb-3 text-sm font-semibold uppercase tracking-[0.12em] text-slate-500">Flow steps</div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="text-sm font-semibold tracking-[0.1em] opacity-80">{step.no}</div>
  ...
  return (
    <section className="rounded-[1.75rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
      <div className="border-b border-slate-200 bg-slate-50 p-5">
        ...
        <p className="mt-2 text-base text-slate-600">{step.subtitle}</p>
      </div>
    ...
    <section className="rounded-[1.75rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
      ...
      <div className="grid gap-4 p-5 xl:grid-cols-2">
        <article className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="text-base font-semibold uppercase tracking-[0.12em] text-slate-500">Why this is hard</h3>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="text-base font-semibold uppercase tracking-[0.12em] text-slate-500">Behavioral risk</h3>
...
export function FitScene({ step, activeIndex, setActiveIndex }) {
  return (
    <section className="rounded-[1.75rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
      <div className="border-b border-slate-200 bg-slate-50 p-5">
        <p className="text-sm font-semibold uppercase tracking-[0.12em] text-slate-500">TrustVC fit · step {step.no}</p>
        <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{step.title}</h2>
      </div>
      ...
      <div className="grid gap-4 p-5 xl:grid-cols-3">
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="text-base font-semibold uppercase tracking-[0.12em] text-slate-500">Remaining build</h3>
...
function TechnicalReferencesPanel({ step }) {
  ...
  const grouped = technicalReferenceStatuses
    .map((status) => [status, references.filter((item) => item.status === status)])
  ...
  return (
    <section className="border-t border-slate-200 p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-base font-semibold uppercase tracking-[0.12em] text-slate-500">Technical references</h3>
          ...
          <details key={status} className="rounded-xl border border-slate-200 bg-slate-50 p-3" open={status === step.status}>
            <summary className="cursor-pointer list-none">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-sm font-semibold text-slate-600">{items.length} capability pointers</span>
              </div>
            </summary>
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-2">
              {items.map((item) => (
                <article key={`${item.stepId}-${item.status}-${item.capability}`} className="rounded-xl border border-slate-200 bg-white p-3">
                  <h4 className="text-sm font-semibold uppercase tracking-[0.1em] text-slate-500">{item.capability}</h4>
                  <p className="mt-2 text-sm leading-7 text-slate-700">{item.technicalReference}</p>
...
function SceneCard({ title, body, compact = false }) {
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4">
      <h3 className="text-base font-semibold uppercase tracking-[0.12em] text-slate-500">{title}</h3>
  ...
  return (
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-900/5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="text-base font-semibold uppercase tracking-[0.12em] text-slate-500">Step glossary</h3>
        <input
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Search terms in this step..."
          className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-base outline-none transition focus:border-slate-400 focus:bg-white"
        />
      </div>
      ...
      <div className="mt-4 grid gap-3 xl:grid-cols-2">
        {filtered.slice(0, 6).map((item) => (
          <article key={item.term} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
            ...
            <p className="mt-1 text-sm leading-7 text-slate-600">{item.appMeaning}</p>
          </article>
        ))}
      </div>
    </section>
